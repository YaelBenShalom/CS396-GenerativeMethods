...
masks.rainbar = function( p) {
	...
	for (var i = 0; i < outlineCount; i++) {
		...
		let weight = SLIDER.thickness*8 + 1
		let hue = SLIDER.color*360

		p.strokeWeight(weight)
		p.stroke(hue, 100*SLIDER.saturation, 50, .8*opacity)
	...
	face.sideOrder.forEach((side) => {
		...
		hand.forEach(h => {
			h.fingers.forEach(finger => {
				drawNeonContour(p, finger, [150, 100, 50], 5, false)
	...
	face.sideOrder.forEach((side) => {
		...
		side.eyeRings.forEach((eyeRing,eyeIndex) => {
			if (eyeIndex === 4) {
				p.fill(0)
				// p.strokeWeight(SLIDER.eyebrows*10+1)
	...
	face.mouth.forEach((mouthLine,mouthIndex) => {
		if (mouthIndex > 2) {
			...
			let h = (40 + 70*mouthIndex + 80*t)%360
