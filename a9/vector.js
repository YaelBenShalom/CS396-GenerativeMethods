...
((root, factory) => {
  ...
  class Vector extends Array {
    constructor() {
      ...
      for (var i = 0; i < arguments.length; i++) {
        if (isNaN(arguments[i]))
          throw `NaN argument for Vector constructor: ${
            arguments[i]
          }(${typeof arguments[i]})`;
      }
    }
    ...
    setToLerp(v0, v1, pct) {
      if (v0.coords) v0 = v0.coords;
      if (v1.coords) v1 = v1.coords;
      ...
      for (var i = 0; i < this.length; i++) {
        for (var j = 0; j < arguments.length; j++) {
          if (isNaN(arguments[j][i])) {
            throw `NaN element in added vector ${arguments[j]}: ${
              arguments[j][i]
    ...
    drawArrow({
      ...
      p.beginShape();
      p.vertex(0, 0);
      p.vertex(-arrowSize * 1.2, arrowSize * 0.5);
