...
let app = {
	...
	draw(p) {
		...
		if (app.maskInstance) {
			if (!controls.paused)
				app.maskInstance.update(t, dt, p.frameCount)	
...
document.addEventListener("DOMContentLoaded", function(){
	...
	new Vue({
		...
		template: `
		<div id="controls">
			<button class="randomized-button" @click="controls.playingRecordedData = !controls.playingRecordedData">Pre-recorded {{controls.playingRecordedData?"⏸":"▶️"}}</button>
			<button class="randomized-button" @click="toggleHandsFree">Real-time {{controls.playingRecordedData?"▶️":"⏸"}}</button>
