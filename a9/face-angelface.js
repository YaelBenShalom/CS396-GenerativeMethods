masks.angelFace = function(p) {

	let t = p.millis()*.001

	p.background(0, 0, 0, .04)
	...
	face.sideOrder.forEach(side => {
		...
		function drawFeather(index, pct) {
			...
			let waveOffset = 1 + 2*SLIDER.thickness
			let pctControl0 = .2 + .4*SLIDER.thickness
			let pctControl1 = .4 + .4*SLIDER.thickness
			...
			p.noStroke()
			p.noFill()
			let hue = (380*SLIDER.color + 60*index + 170*noise(1*t + 4*pct))%360
			p.fill(hue, 110*SLIDER.saturation, 30 + 60*noise(index + 1.2*t + 3*pct), .5)
		...
		p.beginShape()
		let eyeBrowLift = SLIDER.eyebrows - 1
