...
masks.noseFace = function(p) {
	...
	face.sideOrder.forEach(side => {
		...
		let brow = side.eyeRings[0].slice(2, 8).map(pt => {
			...
			pt1.setToLerp(side.eye, pt, SLIDER.eyebrows*2)
			return pt1
		})

		p.strokeWeight(4*SLIDER.thickness)
		drawContour(p, brow)

		p.strokeWeight(2*SLIDER.thickness)
		p.fill(side.index*20 + 100*SLIDER.color + 100, 100*SLIDER.saturation + 20, 70, .8)
		drawStrip(p,side.nose[0],side.nose[1])
		p.fill(side.index*20 + 100*SLIDER.color + 100, 100*SLIDER.saturation + 20, 90, .7)
