...
class FireflysParticles {
	...
	update(p) {
		...
		this.particles.forEach(pt => {
			...
			let windStrength = (SLIDER.wind * 500) * p.noise(t * 0.5)
			
			if (SLIDER.wind < 0.5) {
				windStrength = ((SLIDER.wind - 0.5) * 1000) * p.noise(t * 0.5)
			}
			...
			pt.wind.setToPolar(wanderStrength, wanderDirection)
	...
	draw(p) {
		...
		this.particles.forEach(pt => {
			...
			p.noStroke()
			p.fill(60, 100, 60)
			p.circle(...pt.position, 8)

			p.noStroke()
			p.fill(60, 100, 60, 0.15)
			p.circle(...pt.position, 20)

			p.noStroke()
			p.fill(60, 100, 60, 0.05)
			p.circle(...pt.position, 40)
