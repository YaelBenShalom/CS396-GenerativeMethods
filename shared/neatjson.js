(function (exports) {
  ...
  function neatJSON(value, opts) {
    ...
    var apad = repeat(" ", opts.arrayPadding),
      ...
      comma =
        repeat(" ", opts.beforeComma) + "," + repeat(" ", opts.afterComma),
      colon1 =
        repeat(" ", opts.beforeColon1) + ":" + repeat(" ", opts.afterColon1),
      colonN =
        repeat(" ", opts.beforeColonN) + ":" + repeat(" ", opts.afterColonN);
      ...
      if (o === null || o === undefined) return indent + "null";
        ...
        if (typeof o === "number") {
          if (o === Infinity) {
            ...
            return (
              indent +
              (isFloat && "decimals" in opts
                ? o.toFixed(opts.decimals)
                : o + "")
            );
          }
          ...
          if (opts.short) {
            ...
            if (opts.aligned) {
              ...
              for (var i = keyvals.length; i--; )
                keyvals[i][0] = padRight(padding, keyvals[i][0]);
            }
            for (var i = keyvals.length; i--; ) {
              var k = keyvals[i][0],
                v = keyvals[i][1];
            ...
            var keyvals = sortedKV.map(function (kvs) {
