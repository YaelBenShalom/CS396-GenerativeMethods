...
const faceIndexes = {
  ...
  sides: [
    // RIGHT
    {
      nose: [
        [
          67, 69, 66, 65, 221, 189, 245, 188, 174, 236, 134, 220, 239, 20, 242,
          141, 94,
        ],
    ...
    // LEFT
    {
      nose: [
        [
          297, 299, 296, 295, 441, 413, 412, 399, 456, 363, 440, 459, 250, 462,
          370, 94,
        ],
...
for (var i = 0; i < 2; i++) {
  ...
  hand[i].fingers = faceIndexes.fingers.map((finger) =>
    finger.map((index) => hand[i].points[index])
  );
  ...
  face.sides[i] = {
    ...
    eyeRings: faceIndexes.sides[i].eyeRings.map((ring) =>
      ring.map((index) => {
        return face.points[index];
      })
    ),

    nose: faceIndexes.sides[i].nose.map((ring) =>
      ring.map((index) => {
        return face.points[index];
      })
    ),
    faceRings: faceIndexes.sides[i].faceRings.map((ring) =>
      ring.map((index) => {
        return face.points[index];
      })
    ),
    noseToEar: new Vector(),
    ear: faceIndexes.sides[i].faceRings[0]
      .slice(7, 10)
      .map((index) => face.points[index]),
  };
}
...
// Create mouth lines
face.mouth = faceIndexes.mouth.map((mouthLine) =>
  mouthLine.map((index) => face.points[index])
);
...
function applyHandsFreeData(data) {
  ...
  function setPoint(pt, meshPt) {
    let x = (0.5 - meshPt.x) * canvasW;
    let y = (meshPt.y - 0.5) * canvasH;
    pt.setTo(x, y);
...
function initHandsFree() {
  if (handsfree === undefined) {
    ...
    handsfree.use("logger", (data) => {
      ...
      if (!controls.playingRecordedData) applyHandsFreeData(data);
    });
